...
input_price_usd_per_M = 3.0 # Price for Claude 3 Sonnet input
output_price_usd_per_M = 15.0 # Price for Claude 3 Sonnet output
...
def obtener_cambios_openai(contexto, instruccion_usuario, coder_model, carpeta_proyecto, userId, previous_response, image_path):
    ...
    while retry_count < max_retries:
        try:
            ...
            # Prepare messages for Anthropic (can handle mixed text and images)
            anthropic_messages_content = [{"type": "text", "text": prompt_text}]
            ...
            if image_path and os.path.exists(image_path):
                try:
                    ...
                    if mime_type:
                        ...
                        anthropic_messages_content.append({
                            "type": "image",
                            "source": {
                                "type": "base64",
                        ...
                        print(f"Warning: Unsupported image file type for Anthropic: {image_path}", file=sys.stderr)
                except Exception as e:
                    print(f"Error processing image file {image_path} for Anthropic: {e}", file=sys.stderr)

            messages_for_anthropic = [
                {"role": "user", "content": anthropic_messages_content}
            ]

            # Anthropic API Call with streaming
            full_content = ""
            input_tokens_anthropic = 0
            output_tokens_anthropic = 0

            # Use messages.stream() for long requests as required by Anthropic API error
            with client_anthropic.messages.stream(
                ...
                for chunk in stream:
                    if isinstance(chunk, anthropic.types.MessageStartEvent):
                        input_tokens_anthropic = chunk.message.usage.input_tokens
                        ...
                        output_tokens_anthropic = chunk.usage.output_tokens
            end_time = time.time()
            duration = end_time - start_time

            if full_content:
                try:
                    _update_tokens_usage(input_tokens_anthropic, output_tokens_anthropic, carpeta_proyecto, coder_model, userId, duration)
                except Exception as e:
                    print(f"Error updating token usage for Anthropic: {e}", file=sys.stderr)
                return full_content
            else:
                raise AttributeError("Empty response from Anthropic API.")

        except (TypeError, AttributeError, IndexError) as e:
            last_exception = e
            retry_count += 1
            print(f"Attempt {retry_count}/{max_retries} failed: Error processing response - {e}", file=sys.stderr)
            if retry_count < max_retries:
                print("Retrying in 1 second...", file=sys.stderr)
                time.sleep(1)
            else:
                print(f"Max retries reached. Error processing API response: {e}", file=sys.stderr)
                return ""
        except Exception as e:
            last_exception = e
            retry_count += 1
            ...
            if retry_count < max_retries:
                ...
                print(f"Max retries reached. Error al obtener cambios de la API: {e}", file=sys.stderr)
                return ""
