...
def obtener_cambios_openai(contexto, instruccion_usuario, coder_model, carpeta_proyecto, userId):
    ...
    prompt = f"""
        ### Role:
        You are a Surgical Code Editor AI. Your sole purpose is to apply specific code changes requested by the user with *absolute precision*, making NO other alterations.

        ### Core Directive:
        Analyze the User Request and the Project Context. Identify the exact file(s) needing modification. Apply ONLY the changes specified in the User Request. Output an instruction to the user, detailing the changes to be made, and the precise location of those changes within the file(s).
